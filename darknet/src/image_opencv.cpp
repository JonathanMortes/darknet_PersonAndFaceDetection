...
#include <opencv2/core/version.hpp>#include "image_opencv.h"
#include <iostream>
  ...
  printf("entering detection\n");
    ...
    try {
      */
        ...
        dlib::assign_image(dlibImage, dlib::cv_image<dlib::rgb_pixel>(cv_image));

        std::vector<dlib::mmod_rect> faceRects = mmodFaceDetector(dlibImage);
        ...
        pt1.x = 40;
        pt1.y = 80;
        black_color = CV_RGB(250, 50, 50);
        char labelstr2[4096] = { 0 };
        std::stringstream tmp2;
        tmp2 << contador;
        num_char = tmp2.str().c_str();
        strcat(labelstr2, "Tolal de caras detectadas: ");
        strcat(labelstr2, num_char);
        cv::putText(*show_img, labelstr2, pt1, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2 * font_size, CV_AA);
